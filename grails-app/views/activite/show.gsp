	...
	<body>
		...
		<div id="show-activite" class="content scaffold-show" role="main">
			...
			<ol class="property-list activite">
				...
				<g:if test="${activiteInstance?.responsable}">
				<li class="fieldcontain">
					<span id="responsable-label" class="property-label"><g:message code="activite.responsable.label" default="Responsable" /></span>
					
						<span class="property-value" aria-labelledby="responsable-label"><g:link controller="utilisateur" action="show" id="${activiteInstance?.responsable?.id}">${activiteInstance?.responsable?.encodeAsHTML()}</g:link></span>
